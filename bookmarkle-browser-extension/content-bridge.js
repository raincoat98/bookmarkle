...
// 중복 주입 방지: 이미 주입되었는지 확인하고 즉시 종료
(function () {
  if (window.__BOOKMARKLE_CONTENT_BRIDGE_LOADED__) {
    // 이미 로드된 경우 조용히 종료 (경고 로그 제거)
    return;
  }
  window.__BOOKMARKLE_CONTENT_BRIDGE_LOADED__ = true;
  ...
  function safeSendMessageToExtension(message) {
    ...
    try {
      ...
      console.warn(
        "[content] Failed to send message (will queue if needed):",
        error
      );
      return false;
    }
  ...
  function queueMessageIfNeeded(runtimeMessageType, data) {
    ...
    if (data?.type === "AUTH_STATE_CHANGED") {
      pendingByType.set(runtimeMessageType, {
  ...
  function isKnownValidContext() {
    ...
    return (
      Date.now() - contextState.lastValidTimestamp < CONTEXT_INVALIDATION_TTL_MS
    );
  }
  ...
  function resolvePendingAuthRequests(payload) {
    ...
    requests.forEach((pending) => {
      ...
      pending.sendResponse({
        ok: true,
        payload: payload ?? lastAuthPayload ?? null,
      });
    });
  }
  ...
  function isAllowedOrigin(origin) {
    ...
    try {
      ...
      if (
        hostname === "bookmarkle.app" ||
        hostname.endsWith(".bookmarkle.app")
      ) {
        return true;
      }
      if (
        hostname === "bookmarkle.com" ||
        hostname.endsWith(".bookmarkle.com")
      ) {
        return true;
      }
    } catch {
  ...
  window.addEventListener("message", (event) => {
    ...
    if (data.type === "EXTENSION_DETECTION_PING") {
      markContextValid();
      // 익스텐션 새로고침 시 캐시된 인증 상태 초기화
      // background에서 최신 상태를 다시 보내줄 것임
      lastAuthPayload = null;
      forwardPayloadTracker.clear();
    ...
    if (shouldDedup) {
      const lastPayloadForward =
        forwardPayloadTracker.get(payloadTrackerKey) ?? 0;
    ...
    const ok = safeSendMessageToExtension({
      type: runtimeMessageType,
      payload: data,
    });
    if (!ok) {
  ...
  if (isExtensionContextValid()) {
    ...
    chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
      if (msg.type === "EXTENSION_EVENT_TO_WEB") {
        console.log(
          "[content] EXTENSION_EVENT_TO_WEB received from extension:",
          msg.eventType,
          msg.payload
        );
        window.postMessage(
      ...
      if (msg.type === "WEB_AUTH_STATE_CHANGED") {
        console.log(
          "[content] WEB_AUTH_STATE_CHANGED received from extension:",
          msg.payload
        );
        // 로그아웃 시 명시적으로 null로 설정
        if (!msg.payload || !msg.payload.user) {
          lastAuthPayload = null;
        } else {
          lastAuthPayload = msg.payload;
        }
        // 필요시 웹에 브로드캐스트 가능 (예시)
        window.postMessage(
          {
      ...
      if (msg.type === "REQUEST_WEB_AUTH_STATE") {
        console.log(
          "[content] REQUEST_WEB_AUTH_STATE received from background. hasCache?",
          !!lastAuthPayload
        );
        if (lastAuthPayload) {
